          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                to={link.href}
                className={`text-sm font-medium px-4 py-2 rounded-full transition-all duration-300 ${location.pathname === link.href
            ...
            <nav className="flex flex-col gap-2 bg-card rounded-xl p-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-sm font-medium px-4 py-3 rounded-lg transition-all ${location.pathname === link.href
                      ? "text-gold bg-gold/10"
                      : "text-foreground/70 hover:text-foreground hover:bg-muted/50"
                    }`}
                >
